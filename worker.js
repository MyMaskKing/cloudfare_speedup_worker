...
// 注意：此代码（自定义转发）已不再使用，原因请参考下面内容。

// | 场景                | 自定义转发（new Response） | 直接 fetch（fetch）         |
// |---------------------|-------------------------------|----------------------------|
// | OAuth/SSO 登录      | 只显示你写的提示页面，无法登录 | 能正常走完整认证流程        |
// | WebSocket/SSE       | 连接失败/无响应               | 能正常建立长连接            |
// | API/接口            | 返回固定内容，无法交互         | 能正常请求和返回数据        |
// | 静态资源            | 只显示自定义内容               | 能正常加载图片/JS/CSS 等    |
// ## 结论
// - **返回自定义页面**：用户永远只能看到你写死的内容，无法与后端真实交互。
// - **直接 fetch**：用户和后端的交互完全保留，体验和直连一致，适合需要完整功能的场景。
