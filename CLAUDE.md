...
2. **查找目标映射**：`getTargetForSubdomain` 先查集中映射表 `SUBDOMAIN_MAP[subdomain]`，未命中回退到旧的单变量写法 `self[subdomain]`，仍未找到则回退到 `{subdomain}.{TARGET_DOMAIN}` 默认规则
...
- `SUBDOMAIN_MAP`：**集中映射表（推荐）**，JSON 字符串 `{"子域名":"目标地址"}`，改一处即可批量增删，复制这段 JSON 即完成导入导出；`parseJsonMap` 安全解析（失败或非对象回退空表）
- `{subdomain}`：**单变量写法（旧，仍兼容）**，子域名作为变量名，值为完整目标地址（例如 `blog` → `blog.example.com`）

> **映射查找优先级**：`getTargetForSubdomain` 先查 `SUBDOMAIN_MAP[subdomain]`，未命中回退到 `self[subdomain]`，仍无则回退到默认规则 `{subdomain}.{TARGET_DOMAIN}`。两种写法可共存，便于从旧配置平滑迁移。
